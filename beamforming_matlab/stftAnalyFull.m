function [X, Xfull, param] = stftAnalyFull(x, param)
    ...
    nFrame = ceil((param.siglen - length(param.awin)) / shift) + 1;
    ...
    for i = length(x(:)):-1:1
        ...
        for k = 1:nFrame
            ...
            for f = 1:(length(param.awin) / 2) + 1
                %         Xfull(:,:,k,f) = X(:,k,f).*X(:,k,f)';
    ...
    function S = local_stft(s, awin, shift)
        ...
        %   Sfull: matrix of nFreq * nFreq * nFrame

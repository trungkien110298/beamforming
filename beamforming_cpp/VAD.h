...
{
    int num_samples = audio.rows();
    int num_channels = audio.cols();
    int win_size = round(fs * win_dur);
    int hop_size = round(fs * hop_dur);
    
    int num_frames = floor((num_samples - win_size) / hop_size);
    MatrixXi result(num_frames, num_channels);
    FILE *f = freopen("/home/kienpt/Documents/Beam/stream_audio/VAD.txt", "r", stdin);
